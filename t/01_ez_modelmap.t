...
is scalar( grep { not defined $_ } map { $_->{is_color} } @$info ), 1;

cmp_ok scalar( grep { $_ } map { $_->{gif} } @$info ), '>', 30, 'gif';
is scalar( grep { not defined $_ } map { $_->{gif} } @$info ), 25;
...
is scalar( grep { not defined $_ } map { $_->{jpeg} } @$info ), 18;
...
is scalar( grep { not defined $_ } map { $_->{jpeg} } @$info ), 18, 'no flash lite';
